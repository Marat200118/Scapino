...
void checkReader(MFRC522 &reader, const char *readerName, int ringIndex) {
  ...
  if (cardDetected) {
    ...
    setRingColor(ringIndex, CRGB::Green);
  } else {
    nfcDoc["reader"] = readerName;
    nfcDoc["UID"] = "No card present";

    // Turn off the LED ring corresponding to the reader
    setRingColor(ringIndex, CRGB::Red);
  }
...
void setRingColor(int ringIndex, CRGB color) {
